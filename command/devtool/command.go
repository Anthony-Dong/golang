...
import (
	...
	"github.com/anthony-dong/golang/command/codegen"
	...
	"github.com/anthony-dong/golang/command/static"
	"github.com/anthony-dong/golang/command/strace"
	"github.com/anthony-dong/golang/command/tcp"
	"github.com/anthony-dong/golang/command/trimer"
	"github.com/anthony-dong/golang/command/upload"
	"github.com/anthony-dong/golang/command/wal"
)
...
func NewCommand(config *command.AppConfig, ops ...func(cmd *cobra.Command)) (*cobra.Command, error) {
	...
	if err := command.AddCommand(cmd, codegen.NewCommand); err != nil {
		return nil, err
	}
	...
	if err := command.AddCommand(cmd, wal.NewCommand); err != nil {
		return nil, err
	}
	if err := command.AddCommand(cmd, trimer.NewCommand); err != nil {
		return nil, err
	}
	if err := command.AddCommand(cmd, static.NewCommand); err != nil {
		return nil, err
	}
	return cmd, nil
}
