...
func (f *OssUploadFile) GetPutPath(config *command.OSSConfig) string {
	if f.DstFile != "" {
...
func (f *OssUploadFile) GetOSSUrl(config *command.OSSConfig) string {
	path := f.GetPutPath(config)
	return fmt.Sprintf("https://%s/%s", config.UrlEndpoint, path)
}

// NewBucket 创建桶.
func NewBucket(ossConfig *command.OSSConfig) (*oss.Bucket, error) {
...
// PutFile 上传文件.
func (f *OssUploadFile) PutFile(bucket *oss.Bucket, ossConfig *command.OSSConfig) error {
