...
func (c *CheckFileCanHexoResult) WriteFile() error {
	...
	if needWrite {
		logs.Info("[Hexo] 发现MD5比较不一致需要重新写入到源文件, 文件: %s", c.FileName)
		if err := ioutil.WriteFile(c.FileName, data, utils.DefaultFileMode); err != nil {
			return err
		}
	}
	c.ContentData = data
...
func buildHexo(ctx context.Context, dir string, targetDir string, firmCode []string, ignore []string) error {
	...
	logs.Debug("[Hexo] 开始全部的Markdown的文件, 目录: %s", dir)
	allPage, err := GetAllPage(dir, ignore)
	if err != nil {
		return err
	}
	logs.Debug("[Hexo] 获取全部的Markdown文件成功, 目录: %s, 总数: %d", dir, len(allPage))

	logs.Debug("[Hexo] 开始全部的Target-Markdown的文件, 目录: %s", targetDir)
	targetPage, err := GetAllMarkDownPage(targetDir)
	if err != nil {
		return err
	}
	logs.Debug("[Hexo] 获取全部的Target-Markdown文件成功, 目录: %s, 总数: %d", targetDir, len(targetPage))
		...
		func(fileName string) {
			...
			defer func() {
				if err := recover(); err != nil {
					logs.Error("[Hexo] 运行期间发现 panic, 文件: %s, 异常: %v", fileName, err)
				}
				wg.Done()
				if needEnd {
					logs.Debug("[Hexo] 结束操作文件, 文件: %s, 目标文件: %s", getRelativePath(fileName), getRelativePath(targetFile))
				}
			}()
			// 检测是否是hexo
			result, err := CheckFileCanHexo(fileName, dir)
			if err != nil {
				logs.Error("[Hexo] 检测文件是否是hexo文件发现异常, 文件: %s, 异常: %v", fileName, err)
				return
			}
			if result == nil {
				return
			}
			logs.Debug("[Hexo] 开始操作文件, 文件: %s", getRelativePath(fileName))
			if !result.HasAbstract {
				logs.Warn("[Hexo] 警告, 发现没有摘要, 文件: %s", fileName)
			}
			needEnd = true
			// 开始检测是否有公司代码
			if err := CheckFileHasFirmCode(fileName, result.Content, firmCode); err != nil {
				logs.Error("[Hexo] 检测公司代码失败, 异常: %s, 文件: %s", err, fileName)
				return
			}

			// 检测文件格式写入（格式化原文件）
			if err := result.WriteFile(); err != nil {
				logs.Error("[Hexo] 检测原文件格式失败, 异常: %s, 文件: %s", err, fileName)
				return
			}
			...
			writeTargetFile := func() {
				if err := ioutil.WriteFile(targetFile, result.ContentData, utils.DefaultFileMode); err != nil {
					logs.Error("[Hexo] 发现需要写入到Hexo的post目录文件发现了异常, 文件: %s, 异常: %v", targetFile, err)
					return
				}
			...
			if !targetPageSet.Contains(targetFile) {
				logs.Info("[Hexo] 发现Hexo的post目录文件不存在需要写入的文件, 目标文件: %s, 源文件: %s", targetFile, fileName)
				// 直接写入
				writeTargetFile()
			} else {
				// 否则读出来比较MD5是否相同，再写入！
				...
				if err != nil {
					logs.Error("[Hexo] 发现读取Hexo的post目录文件发现了异常, 文件: %s, 异常: %v", targetFile, err)
					return
				}
				if codec.Md5HexString(result.ContentData) != codec.Md5HexString(readBody) {
					logs.Info("[Hexo] 发现读取Hexo的post目录文件和原文件MD5值不一样, 需要重写, 文件: %s,  源文件: %s", getRelativePath(targetFile), getRelativePath(fileName))
					writeTargetFile()
				}
			}
	...
	// delete
	logs.Info("[Hexo] 操作脚本完成, 一共写入: %d, 总页数: %d", needWriteNewTargetPage.Size(), newTargetPage.Size())
	...
	logs.Info("[Hexo] 操作脚本完成需要删除文件: %d", targetPageSet.Size())
	...
	for _, elem := range targetPageSet.ToSlice() {
		logs.Info("[Hexo] 删除文件, 文件: %s", elem)
		if err := os.Remove(elem); err != nil {
			logs.Error("[Hexo] 删除文件失败, 文件: %s, 异常: %s", elem, err)
			return err
		}
	}
	...
	for index, line := range content {
		for _, elem := range firmCode {
			if strings.Contains(line, elem) {
				logs.Warn("[Hexo] 发现公司代码, 文件: %s, 公司代码: %s, 原文: %s", fileName, elem, line)
				newElem := utils.NewString('x', len(elem))
...
func CheckFileCanHexoPre(fileName string) bool {
	...
	count := 0
	if err := utils.ReadLineByFunc(file, func(line string) error {
		if line == "" {
			return nil
		}
		count++
...
func CheckFileCanHexo(fileName string, fileParentPath string) (*CheckFileCanHexoResult, error) {
	...
	// 文件修改时间
	if fileConfig.Date == "" {
		fileConfig.Date = fileInfo.ModTime().Format(utils.FormatTimeV1)
	}
...
func GetAllPage(dir string, ignoreLine []string) ([]string, error) {
	...
	if utils.ExistFile(ignoreFileName) {
		var err error
		ignore, err = git.CompileIgnoreFileAndLines(ignoreFileName, ignoreLine...)
		if err != nil {
			logs.Error("[GetAllPage] load ignore file: %s find err: %v", ignoreFileName, err)
			return nil, err
		}
		logs.Info("[GetAllPage] load ignore file: %s, config: %s, success!", ignoreFileName, utils.ToString(ignoreLine))
	} else {
		ignore = git.CompileIgnoreLines(ignoreLine...)
		logs.Info("[GetAllPage] load ignore config: %s success!", utils.ToString(ignoreLine))
	}

	return utils.GetAllFiles(dir, func(filePath string) bool {
...
func GetAllMarkDownPage(dir string) ([]string, error) {
	return utils.GetAllFiles(dir, func(fileName string) bool {
