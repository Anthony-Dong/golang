...
func NewCommand(name string, ops ...tcpdump.DecodeOption) (*cobra.Command, error) {
	var (
		filename       string
		verbose        bool
...
2. 离线分析:
 ...
 - 分析流量: %[1]s -r xxx.pcap`, joinCommand(name)),
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			option := tcpdump.NewDecodeOptions(ops...)
			if option.MsgWriter == nil {
				...
				logs.CtxInfo(ctx, "filter msg type: %s", utils.ToJson(filterMsgTypes))
			...
			return DecodePacketSource(ctx, source, option)
		},
	}
	cmd.Flags().BoolVarP(&verbose, "verbose", "v", false, "开启DEBUG模式, 会打印一些DEBUG信息")
	cmd.Flags().StringVarP(&filename, "file", "r", "", "读取tcpdump抓取的pcap文件")
	cmd.Flags().StringArrayVarP(&filterMsgTypes, "filter", "f", []string{}, "过滤消息类型(thrift/http/http2/log/tcpdump)")
	return cmd, nil
}

func NewPacketSource(filename string, options tcpdump.DecodeOptions) (tcpdump.PacketSource, error) {
	if filename != "" {
		return tcpdump.NewFileSource(filename, options)
	}
	if utils.CheckStdInFromPiped() {
		source := tcpdump.NewConsulSource(os.Stdin, options)
		return source, nil
	}
	return nil, fmt.Errorf("no packet source")
}

func GetPacketSourceType(filename string) tcpdump.PacketSourceType {
	if filename != "" {
		return tcpdump.PacketSource_File
	}
	if utils.CheckStdInFromPiped() {
		return tcpdump.PacketSource_Consul
	}
	return tcpdump.PacketSource_Unknown
}

func DecodePacketSource(ctx context.Context, source tcpdump.PacketSource, option tcpdump.DecodeOptions) error {
	...
	for data := range source.Packets() {
		...
		if wait, isOk := data.(tcpdump.WaitPacket); isOk {
			wait.Notify()
		}
	}
	return nil
...
func NewTcpPacket(packet gopacket.Packet, writer tcpdump.MessageWriter) *tcpdump.TcpPacket {
	...
	if L3IsOk && L4IsOK {
		...
		result := tcpdump.HandlerTcp(data.Src, data.Dst, tcp)
		if !result.Is(tcpdump.OutOfOrderStatus) {
		...
		header.WriteString(fmt.Sprintf("%s ", tcpdump.GetRelativeInfo(data.Src, data.Dst, tcp)))
