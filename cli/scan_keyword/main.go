...
func main() {
	dir := ""
	keywordsBase64 := ""
	flag.StringVar(&dir, "dir", ".", "scan dir")
	flag.StringVar(&keywordsBase64, "keywords", "", "scan keywords")
	flag.Parse()
	if keywordsBase64 == "" {
		Infof("未申明任何关键字")
		return
	...
	for _, keyword := range strings.Split(keywordsBase64, ";") {
		decodeString, err := base64.StdEncoding.DecodeString(keyword)
		if err != nil {
			panic(err)
		}
		keywords = append(keywords, string(decodeString))
	}
	if len(keywords) == 0 {
		Infof("未申明任何关键字")
		return
...
// 清楚代码的关键字.
func clear(dir string, keywords []string) {
	...
	gitIgnore, err := CompileIgnoreFileAndLines(filepath.Join(dir, ".gitignore"), "/.git")
	if err != nil {
		panic(err)
	}
	Infof("开始检测代码: %+v, 位置: %s", ToCliMultiDescString(keywords), AbsPath(dir))
	// 获取全部文件
	...
	for _, elem := range allFile {
		// 检测文件
		CheckFile(&wg, keywords, elem)
	}
	// 结束
	wg.Wait()

	Infof("完成检测代码: %+v, 位置: %s, 一共检测 %d 个文件 !", ToCliMultiDescString(keywords), AbsPath(dir), len(allFile))
}
