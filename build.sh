...
function cross_go_build(){
  ...
  CGO_ENABLED=0 GOOS=$1 GOARCH=$2 go build "${build_flag[@]}" -o "output/$1_$2/$binary" "cli/$3/main.go"
}
...
function go_build(){
  binary="$1"
  if [ "$IS_SUBMOD" = "1" ]; then
    cd "cli/$1" && go build "${build_flag[@]}" -o "../../output/$binary" "main.go" && cd -
  else
    go build "${build_flag[@]}" -o "output/$binary" "cli/$1/main.go"
  fi
}
