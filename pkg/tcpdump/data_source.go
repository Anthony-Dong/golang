...
func WithMsgWriter(writer MessageWriter) DecodeOption {
	return func(options *DecodeOptions) {
		options.MsgWriter = writer
	}
}

func WithDecoder(decoder Decoder) DecodeOption {
	return func(options *DecodeOptions) {
...
func (t *ConsulSource) init() {
	...
	go func() {
		for elem := range t.lines {
			...
			t.writer.Write(&TcpdumpPayload{Payload: decode})
		}
		close(t.packets)
	}()
}
