...
import (
	...
	"github.com/anthony-dong/golang/pkg/codec/pb_codec/test"
	"github.com/anthony-dong/golang/pkg/utils"
)

func NewPB2Data(t *testing.T) []byte {
	any, err := anypb.New(&test.TestPB2Data_TestPB2Obj{TInt64: utils.Int64Ptr(1)})
	if err != nil {
		t.Fatal(err)
	}
	result := test.TestPB2Data{
		TString:  utils.StringPtr("1234"),
		TInt64:   utils.Int64Ptr(1),
		TBool:    utils.BoolPtr(true),
		TFix64:   utils.Uint64Ptr(1),
		...
		TMapObj: map[string]*test.TestPB2Data{
			"k1": {
				TString: utils.StringPtr("1234"),
				TInt64:  utils.Int64Ptr(1),
				TBool:   utils.BoolPtr(false),
				TFix64:  utils.Uint64Ptr(1),
			},
			"k2": {
				TString: utils.StringPtr("k2"),
				TInt64:  utils.Int64Ptr(2),
				TBool:   utils.BoolPtr(true),
				TFix64:  utils.Uint64Ptr(1),
			},
		...
		Result: []*test.TestPB2Data_Result{
			{
				Url:      utils.StringPtr("1"),
				Title:    utils.StringPtr("Title"),
				Snippets: []string{"1", "2"},
			},
			{
				Url:      utils.StringPtr("2"),
				Title:    utils.StringPtr("Title2"),
				Snippets: []string{"12", "22"},
			},
		},
		TDouble: utils.Float64Ptr(1.0111),
...
func TestDecodeMessage(t *testing.T) {
	t.Run("test", func(t *testing.T) {
		...
		t.Log(utils.ToJson(message, true))
	})
